...
func (this *IntegratedBP32) Uncompress(in []uint32, inpos *encoding.Cursor, inlength int, out []uint32, outpos *encoding.Cursor) error {
	...
	for s := outpos.Get(); s < outpos.Get() + int(outlength); s += 32*4 {
		...
		bitpacking.IntegratedUnpack(initoffset, in, tmpinpos, out, s, int(mbits1))
		tmpinpos += int(mbits1)
		initoffset = out[s + 31]
		//log.Printf("bp32/Uncompress: out = %v\n", out)

		bitpacking.IntegratedUnpack(initoffset, in, tmpinpos, out, s + 32, int(mbits2))
		tmpinpos += int(mbits2)
		initoffset = out[s + 32 + 31]
		//log.Printf("bp32/Uncompress: out = %v\n", out)

		bitpacking.IntegratedUnpack(initoffset, in, tmpinpos, out, s + 2*32, int(mbits3))
		tmpinpos += int(mbits3)
		initoffset = out[s + 2*32 + 31]
		//log.Printf("bp32/Uncompress: out = %v\n", out)

		bitpacking.IntegratedUnpack(initoffset, in, tmpinpos, out, s + 3*32, int(mbits4))
		tmpinpos += int(mbits4)
